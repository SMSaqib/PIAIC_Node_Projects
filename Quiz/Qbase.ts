...
let db = new sqlite3.Database('Quiz_db.db');
  ...
  async function getQuestions() {
    ...
    return new Promise((resolve, reject) => {
       db.all("SELECT * FROM Question_table WHERE Question IS NOT Null AND Question <>'' ", [], (err, rows:any) => {
        let questions = [];
...
async function select_ques(x:string){
  let options:any = await getValue(x)
...
function filterIndex(array: number[],criteria:string|number): number[] {
  let FailQues: number[] = [];
...
function createTableFromFourArrays( array1: any[],  array2: any[],  array3: any[],  array4: any[]): string[][] {
