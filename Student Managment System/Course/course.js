    ...
    async addCourse() {
        ...
        for (let crs of courses_array) {
            await new Promise((resolve, reject) => {
                this.db.run(sql, [crs[0], crs[1], crs[2], crs[3], crs[4], crs[5], crs[6], crs[7], crs[8], crs[9], crs[10],], (err) => {
                    if (err) {
                        reject(err);
                    }
                    else {
...
FROM Tcourse WHERE ProgramId = ?  AND Semester = ? `;
    ...
    Courses_Charges_bySemester(x, y) {
        return new Promise((resolve, reject) => {
            const sql = `SELECT SUM(CourseCharges) as t
FROM Tcourse WHERE ProgramId = ?  AND Semester = ? `;
            this.db.get(sql, [x, y], (err, rows) => {
                if (err) {
                    reject(err);
                }
                else {
...
// console.table(x)
export { CourseRepository, courseColumnName, courses_array };
