...
interface User {
  ...
  father: string;
  password: string;
  pin: string | number; // Use lowercase 'pin'
}
...
async function ReattemptV(x:number) {
  let FT = await inquirer.prompt([
    {
      name: "conf",
      type: "list",
...
let id_in:number;
let name_in:string;
let father_in :string;
let password_in :string;
let pin_in:string;
let userData_Collection:User
...
do {
  ...
  db.run(
    ...
    [ name_in, father_in, password_in, pin_in],
    (err) => {
      if (err) {
...
async function BP_Customer_Info() { 
  ...
  export {User,Transaction_list, FT_Account_Number,pinVerification ,ReattemptV ,Data_collection,new_User_Regs,BP_Customer_Info,Withdrawl}